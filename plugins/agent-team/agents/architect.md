...
**Step 0**: load `artifact-templates` and `diagram-excalidraw` via the Skill tool.
...
6. **Write ADRs** for every decision that was not obvious.
7. **Draw the diagram** that shows the mechanism, not just the boxes.
